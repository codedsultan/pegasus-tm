...
const MenuItemWithChildren = ({ item, linkClassName, subMenuClassNames, activeMenuItems, toggleMenu }: SubMenus) => {
	...
	return (
		<li className={`menu-item ${open ? 'menuitem-active' : ''}`}>
			<Link href="" className={`${linkClassName} ${activeMenuItems!.includes(item.key) ? 'open' : ''}`} aria-expanded={open} data-menu-key={item.key}
                onClick={(e) => {
                    e.preventDefault();
                    toggleMenuItem();
                }}
...
const MenuItemLink = ({ item, className }: SubMenus) => {
	return (
		<Link href={item.url!} target={item.target} className={`menu-link side-nav-link-ref ${className}`} data-menu-key={item.key}>
			{item.icon && (
				<span className="menu-icon">
					<i className={item.icon} />
				</span>
